...
filtered_csv_data = csv_data[csv_data['Bölüm'].str.contains('Makine Mühendisliği')]
...
for course, group in grouped_by_course:
    ...
    for _, time_group in grouped_by_time:
        if  'Makine Mühendisliği (Ö.Ö-İÖ)' in time_group['Bölüm'].values:
            education_type = 'Gündüz ve Gece'
        else:
            education_type = 'Gündüz' if 'Makine Mühendisliği (Ö.Ö)' in time_group['Bölüm'].values else 'Gece'
...
final_corrected_json_file_path = 'makine.json'
